...
const home = async (req, res) => {
  ...
  if (user.role === 'student') {
    ...
    assignments = await Assignment.find().populate('studentId').populate('professorId').populate('hodId').sort({ createdAt: -1 });
    stats = { totalAssignments: assignments.length, totalUsers: await User.countDocuments() };
  }
